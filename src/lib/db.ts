...
const resolveLocation = (locationId: string) => store.locations.find((l) => l.id === locationId) ?? null;
...
export const db = {
  ...
  course: {
    async findMany(args?: any) {
      ...
      if (args?.where?.repId) courses = courses.filter((c) => c.repId === args.where.repId);
      courses = args?.orderBy?.createdAt === "desc" ? descByCreatedAt(courses) : courses;
      ...
      return courses.map((course) => {
        ...
        if (args?.include?.sessions) {
          out.sessions = sessions
            ...
            .map((s) => ({
              ...s,
              location: args.include.sessions.include?.location ? resolveLocation(s.locationId) : undefined,
    ...
    async findFirst(args?: any) {
      const where = args?.where ?? {};
      return (
        store.courses.find((c) => Object.entries(where).every(([k, v]) => (c as any)[k] === v)) ?? null
      );
    },
  ...
  location: {
    async findMany(args?: any) {
      ...
      if (args?.orderBy?.name === "asc") return ascByName(locations);
      if (args?.orderBy?.createdAt === "desc") return descByCreatedAt(locations);
      return locations;
    },
    async create(args?: { data?: any }) {
      const data = args?.data ?? {};
  ...
  attendanceSession: {
    async findMany(args?: any) {
      ...
      if (args?.where?.course?.repId) {
        sessions = sessions.filter((s) => resolveCourse(s.courseId)?.repId === args.where.course.repId);
      }
      sessions = args?.orderBy?.createdAt === "desc" ? descByCreatedAt(sessions) : sessions;
